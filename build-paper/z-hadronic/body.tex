...
are discussed in Sections~@@TOK:REF:sec:limitations@@ and~@@TOK:REF:sec:reproducibility@@.
\end{abstract}
\section{@@TOK:HEADID:paper-head-1@@@@TOK:SECNUM:1. @@Introduction}
...
couplings to Standard Model (SM) particles @@TOK:CITE:abercrombie_dm_forum@@.
...
radiate an on-shell $Z$ boson @@TOK:CITE:backovic_schannel,neubert_monoz@@. The
...
In a preceding study @@TOK:CITE:rasineni_nsf_monoz@@, we demonstrated a
...
(NSFs) @@TOK:CITE:durkan_nsf,papamakarios_flows@@---two channel-specific SM
...
flow (CFM-CNF) @@TOK:CITE:lipman_flow_matching,chen_neural_ode@@. Whereas an NSF
...
Hutchinson's stochastic estimator @@TOK:CITE:grathwohl_ffjord@@. This
continuous-time formulation is trained with the simulation-free conditional
flow-matching objective @@TOK:CITE:lipman_flow_matching@@, which regresses the
...
zero-imputation @@TOK:CITE:rasineni_nsf_monoz@@; here we instead preserve the
...
@@TOK:CITE:cms_htmht_2015d@@, corresponding to a validated recorded integrated
luminosity of $2.256382381~\invfb$ @@TOK:CITE:cms_data_policy,cms_lumi_guide@@,
and follows the open-data research practices advocated by the CMS open-data
community @@TOK:CITE:lassila_cms_opendata,bellis_cms_dp@@. Dark-matter signal
samples are generated with the \texttt{DMsimp\_s\_spin1} simplified-model
implementation of an $s$-channel spin-1 mediator
@@TOK:CITE:abercrombie_dm_forum,backovic_schannel@@ using a reproducible
\textsc{MadGraph5\_aMC@NLO} + \textsc{Pythia8} + \textsc{Delphes} toolchain
@@TOK:CITE:alwall_mg5amc,sjostrand_pythia82,delphes3@@. Under the post-fix
...
\section{@@TOK:HEADID:paper-sec-data@@@@TOK:SECNUM:2. @@Data and simulation}

\subsection{@@TOK:HEADID:paper-head-2@@@@TOK:SECNUM:2.1 @@CMS Run 2015D HTMHT open data}

The analysis uses the CMS Run~2015D HTMHT MINIAOD open dataset
@@TOK:CITE:cms_htmht_2015d@@, corresponding to a validated recorded integrated
luminosity of $2.256382381~\invfb$ for runs 256630--260627
@@TOK:CITE:cms_data_policy,cms_lumi_guide@@. Events are read in chunks via
...
(Appendix~@@TOK:REF:app:filecounts@@). In total 444 MINIAOD files containing
...
paths listed in Appendix~@@TOK:REF:app:hlt@@, which comprise $H_T$-based,
...
observables; see Appendix~@@TOK:REF:app:features@@) and are used for the
...
been documented by the CMS open-data team @@TOK:CITE:lassila_cms_opendata@@.
...
NanoAOD to improve usability @@TOK:CITE:lassila_cms_opendata@@. The present
...
@@TOK:CITE:lassila_cms_opendata@@. The luminosity information is taken from the
validated Run~2015D recorded value, following the open-data luminosity
guidance @@TOK:CITE:cms_lumi_guide@@.

\subsection{@@TOK:HEADID:paper-head-3@@@@TOK:SECNUM:2.2 @@Signal generation}
...
@@TOK:CITE:alwall_mg5amc,sjostrand_pythia82,delphes3@@, with jets reconstructed
using the anti-$k_T$ algorithm @@TOK:CITE:cacciari_antikt@@. The anti-$k_T$
algorithm was chosen for its soft-resilient jet boundaries: as shown in the
original paper @@TOK:CITE:cacciari_antikt@@, its jets are conical with radius $R$
...
@@TOK:CITE:abercrombie_dm_forum,backovic_schannel@@, for the process
$pp \to Z\bar{\chi}\chi$ with $Z \to jj$. Three benchmark points,
listed in Table~@@TOK:REF:tab:signal-points@@, are retained for the final
...
the ATLAS/CMS Dark Matter Forum @@TOK:CITE:abercrombie_dm_forum@@, which
...
the $V + \MET$ final states @@TOK:CITE:backovic_schannel,abercrombie_dm_forum@@,
...
s-channel mediator models @@TOK:CITE:backovic_schannel@@. The DMsimp
...
@@TOK:CITE:backovic_schannel@@, and the NLO corrections to DM pair production in
...
@@TOK:CITE:backovic_schannel@@. For the benchmark points retained here, the
...
Table~@@TOK:REF:tab:signal-points@@ should be regarded as lower estimates of the
...
largest corrections in the low-\MET{} region @@TOK:CITE:backovic_schannel@@,
...
\begin{table}[t]
  \centering
  \small
  \caption{@@TOK:CAPT:1@@ Retained dark-matter signal benchmarks. \texttt{DMsimp\_s\_spin1}
  ...
  \begin{tabular}{@{\extracolsep{\fill}}>{\raggedright\arraybackslash}p{4.8cm}llrr@{}}
    \toprule
...
(Appendix~@@TOK:REF:app:hlt@@; \S\ref{sec:trigger-proxy}) is applied to the
...
\subsection{@@TOK:HEADID:paper-head-4@@@@TOK:SECNUM:2.3 @@Z-candidate reconstruction and feature contract}
...
\section{@@TOK:HEADID:paper-head-5@@@@TOK:SECNUM:3. @@Methods}
...
\subsection{@@TOK:HEADID:paper-head-6@@@@TOK:SECNUM:3.1 @@Background density model}

The background density is modelled with a conditional flow-matching
continuous normalizing flow (CFM-CNF) @@TOK:CITE:lipman_flow_matching@@. The
...
covariates. A continuous normalizing flow @@TOK:CITE:chen_neural_ode,
...
@@TOK:CITE:durkan_nsf,rasineni_nsf_monoz@@, the CNF density is evaluated by
numerically integrating Eq.~@@TOK:EQREF:eq:pflow@@, which is computationally more
...
predecessor @@TOK:CITE:rasineni_nsf_monoz@@ reflects several methodological
...
Asimov expected significance @@TOK:CITE:cowan_asymptotic@@. This simplification
...
the leptonic analysis's high-\MET{} tail @@TOK:CITE:rasineni_nsf_monoz@@.
...
@@TOK:CITE:chen_neural_ode@@, which showed that the change in log density under a
...
@@TOK:CITE:chen_neural_ode@@. The normalizing-flow review of Papamakarios et al.
@@TOK:CITE:papamakarios_flows@@ classifies flow architectures into finite
...
@@TOK:CITE:durkan_nsf,rasineni_nsf_monoz@@, the CNF does not require a
...
autoregressive layers @@TOK:CITE:chen_neural_ode,papamakarios_flows@@. The
...
complexity @@TOK:CITE:papamakarios_flows@@; this symmetry is exploited here by
...
@@TOK:CITE:durkan_nsf@@ parameterises the elementwise transformations in coupling
...
@@TOK:CITE:grathwohl_ffjord@@, which showed that the continuous-time
instantaneous change of variables of Chen et al. @@TOK:CITE:chen_neural_ode@@
can be scaled to high-dimensional problems by estimating the Jacobian trace
with Hutchinson's stochastic estimator @@TOK:CITE:grathwohl_ffjord@@. Because the
...
FFJORD @@TOK:CITE:grathwohl_ffjord@@. The learned residual-MLP vector field can
...
the leptonic analysis @@TOK:CITE:durkan_nsf,rasineni_nsf_monoz@@.
...
\end{equation}@@TOK:EQA:eq:fmloss@@
...
\end{equation}@@TOK:EQA:eq:logprob@@
...
projection per ODE evaluation @@TOK:CITE:grathwohl_ffjord@@. The probability-flow
...
\subsection{@@TOK:HEADID:paper-head-7@@@@TOK:SECNUM:3.2 @@Feature contract and preprocessing}
...
\subsection{@@TOK:HEADID:paper-head-8@@@@TOK:SECNUM:3.3 @@Architecture and training}
...
Training minimises Eq.~@@TOK:EQREF:eq:fmloss@@ with \texttt{AdamW}
...
log-likelihood from Eq.~@@TOK:EQREF:eq:logprob@@, with early stopping after six
...
\subsection{@@TOK:HEADID:paper-head-9@@@@TOK:SECNUM:3.4 @@Inference and sensitivity evaluation}

Per-event negative log-likelihoods (NLL) from Eq.~@@TOK:EQREF:eq:logprob@@ serve
...
population, as detailed in Section~@@TOK:REF:sec:sens-methodology@@. Signal
samples are scored only after the offline trigger proxy described in
Section~@@TOK:REF:sec:trigger-proxy@@. The expected-significance calculation,
the minimum-background-yield working point, and the diagnostic closure
studies are described in Sections~@@TOK:REF:sec:sens-methodology@@ and
@@TOK:REF:sec:validation@@.

\section{@@TOK:HEADID:paper-sec-validation@@@@TOK:SECNUM:4. @@Validation}
...
\subsection{@@TOK:HEADID:paper-head-10@@@@TOK:SECNUM:4.1 @@Closure plots (representative set)}
...
Representative panels are shown in Figure~@@TOK:REF:fig:closure-representative@@,
and the full per-feature closure set is provided in
Appendix~@@TOK:REF:app:closures@@.

The hadronic-$Z$ mass panel (top left of
Figure~@@TOK:REF:fig:closure-representative@@) shows that the flow reproduces the
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_m_jj.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_met_pt.png}\\[6pt]
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_HT.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_deltaR_zjj.png}
  \caption{@@TOK:CAPF:1@@ Representative closure plots comparing held-out validation data
  ...
  per-feature closure set is provided in Appendix~@@TOK:REF:app:closures@@.}
  \label{fig:closure-representative}
@@TOK:ENVEND:f1@@\end{figure}

Figure~@@TOK:REF:fig:closure-kinematics@@ shows additional closure panels for the
...
score used in Section~@@TOK:REF:sec:sens-methodology@@.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_pt_Z.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_hadronic_recoil_pt.png}\\[6pt]
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_met_over_sqrtHT.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_abs_mjj_minus_mZ.png}
  \caption{@@TOK:CAPF:2@@ Additional closure panels for kinematic observables entering the
...
@@TOK:ENVEND:f2@@\end{figure}

\subsection{@@TOK:HEADID:paper-head-11@@@@TOK:SECNUM:4.2 @@Before/after imputation-fix comparison}
...
Figure~@@TOK:REF:fig:imputation-comparison@@ shows the post-fix closure panels for
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_jet1_eta.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_jet2_eta.png}\\[6pt]
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_dphi_met_jet1.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_dphi_met_jet2.png}
  \caption{@@TOK:CAPF:3@@ Post-fix closure panels for the four undefined extra-jet angular
...
@@TOK:ENVEND:f3@@\end{figure}

\subsection{@@TOK:HEADID:paper-head-12@@@@TOK:SECNUM:4.3 @@Training convergence and validation NLL}
...
(Eq.~@@TOK:EQREF:eq:logprob@@). Figure~@@TOK:REF:fig:validation-nll@@ shows the
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.92\textwidth]{/research/z-hadronic/assets/images/validation_nll_curve.png}

  \vspace{0.8em}

  \includegraphics[width=0.92\textwidth]{/research/z-hadronic/assets/images/validation_nll_curve_ablation_drop_extrajet.png}
  \caption{@@TOK:CAPF:4@@ Validation negative-log-likelihood curves for the baseline (top)
...
@@TOK:ENVEND:f4@@\end{figure}

\subsection{@@TOK:HEADID:paper-head-13@@@@TOK:SECNUM:4.4 @@Known residual limitation: jet pT delta-plus-continuum}
...
Figure~@@TOK:REF:fig:pt-limitation@@, the closure for \texttt{jet1\_pt} and
...
as a caveat in Section~@@TOK:REF:sec:limitations@@.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_jet1_pt.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_jet2_pt.png}
  \caption{@@TOK:CAPF:5@@ Closure panels for the extra-jet $p_T$ features \texttt{jet1\_pt}
...
@@TOK:ENVEND:f5@@\end{figure}

\section{@@TOK:HEADID:paper-sec-sens-methodology@@@@TOK:SECNUM:5. @@Sensitivity methodology}
...
significances reported in Section~@@TOK:REF:sec:results@@. The per-event negative
log-likelihood (NLL) from Eq.~@@TOK:EQREF:eq:logprob@@ serves as the discriminant:
...
\subsection{@@TOK:HEADID:paper-head-14@@@@TOK:SECNUM:5.1 @@Asimov significance}
...
\end{equation}@@TOK:EQA:eq:asimov@@
...
Cowan et al. @@TOK:CITE:cowan_asymptotic@@, which derived closed-form sampling
...
@@TOK:CITE:cowan_asymptotic@@. The counting formula of Eq.~@@TOK:EQREF:eq:asimov@@ is
...
statistic $q_0$ in the large-sample limit @@TOK:CITE:cowan_asymptotic@@. The
full formal treatment with nuisance parameters would generalise
Eq.~@@TOK:EQREF:eq:asimov@@ to a profile likelihood over the background-density
...
\subsection{@@TOK:HEADID:paper-head-15@@@@TOK:SECNUM:5.2 @@Signal-yield normalization}
...
\begin{equation}
  w_{\mathrm{sig}} = \frac{\sigma\, \mathcal{L}\, \epsilon_{\mathrm{trig}}}{N_{\mathrm{gen}}},
  
\end{equation}@@TOK:EQA:eq:signal-weight@@
where $\sigma$ is the leading-order cross section, $\mathcal{L}$ is the
integrated luminosity, $\epsilon_{\mathrm{trig}}$ is the signal-side trigger-proxy
efficiency, and $N_{\mathrm{gen}}$ is the number of generated signal events used
for normalisation. The signal yield above a threshold is then
$S = w_{\mathrm{sig}}\, N_{\mathrm{sig}}^{>\mathrm{cut}}$, where
$N_{\mathrm{sig}}^{>\mathrm{cut}}$ is the number of simulated signal events whose
NLL exceeds the threshold. The cross sections and generated-event counts are
taken from the signal-generation manifest (Table~@@TOK:REF:tab:signal-points@@),
and the luminosity is the validated Run~2015D recorded value of
$2.256382381~\invfb$ @@TOK:CITE:cms_data_policy,cms_lumi_guide@@.

\subsection{@@TOK:HEADID:paper-head-16@@@@TOK:SECNUM:5.3 @@Discriminant scan and working-point selection}
...
$Z_A$ is computed from Eq.~@@TOK:EQREF:eq:asimov@@. During early audits we found
...
\subsection{@@TOK:HEADID:paper-head-17@@@@TOK:SECNUM:5.4 @@Held-out-only background scoring with reweighting}
...
\begin{equation}
  w_{\mathrm{pop}} = \frac{N_{\mathrm{selected}}}{N_{\mathrm{scored}}},
  
\end{equation}@@TOK:EQA:eq:pop-weight@@
where $N_{\mathrm{selected}}=1{,}439{,}523$ is the post-selection population and
$N_{\mathrm{scored}}=287{,}905$ is the held-out validation count in the baseline
run. The background yield above a threshold is
$B = w_{\mathrm{pop}}\, N_{\mathrm{bg}}^{>\mathrm{cut}}$, where
$N_{\mathrm{bg}}^{>\mathrm{cut}}$ is the number of held-out background events
...
\subsection{@@TOK:HEADID:paper-sec-trigger-proxy@@@@TOK:SECNUM:5.5 @@Signal-side trigger proxy}
...
efficiency is 100\% for each point and $\epsilon_{\mathrm{trig}}=1$ in
Eq.~@@TOK:EQREF:eq:signal-weight@@. This proxy is conservative but does not
replace a full HLT efficiency measurement, which would require a data-driven
turn-on parameterisation.


\section{@@TOK:HEADID:paper-sec-results@@@@TOK:SECNUM:6. @@Results}
...
@@TOK:CITE:cms_htmht_2015d@@. After the hadronic mono-$Z$ preselection the local
...
recorded luminosity provenance @@TOK:CITE:cms_data_policy,cms_lumi_guide@@.

\begin{table}[H]
  \centering
  \caption{@@TOK:CAPT:2@@ Summary of the processed MINIAOD dataset used for extraction.}
...
@@TOK:ENVEND:t2@@\end{table}

Full per-file counts (entries and selected events) are provided in
Appendix~@@TOK:REF:app:filecounts@@ and summarized above.

Significances are computed with the Asimov counting formula
(Eq.~@@TOK:EQREF:eq:asimov@@) described in Section~@@TOK:REF:sec:sens-methodology@@,
...
\begin{table}[t]
  \centering
  \caption{@@TOK:CAPT:3@@ Primary post-fix expected sensitivity for the 32-feature
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{3.1cm}>{\raggedright\arraybackslash}p{3.0cm}rrrrrr}
    \toprule
    ...
    \bottomrule
  \end{tabular}
@@TOK:ENVEND:t3@@\end{table}

Table~@@TOK:REF:tab:primary-results@@ reports the post-fix baseline numbers. The
...
\subsection{@@TOK:HEADID:paper-head-18@@@@TOK:SECNUM:6.1 @@Comparison with the leptonic NSF predecessor}
...
@@TOK:CITE:rasineni_nsf_monoz@@. The two analyses differ in three principal
...
@@TOK:CITE:lipman_flow_matching,chen_neural_ode,grathwohl_ffjord@@.
...
fitted signal strength @@TOK:CITE:rasineni_nsf_monoz@@. The present analysis
instead reports projected Asimov expected significances
@@TOK:CITE:cowan_asymptotic@@ computed from a held-out background score
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/expected_significance_axial_mx10_mv20.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/expected_significance_axial_mx10_mv20_ablation_drop_extrajet.png}\\[6pt]
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/expected_significance_axial_mx50_mv200.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/expected_significance_axial_mx50_mv200_ablation_drop_extrajet.png}\\[6pt]
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/expected_significance_vector_mx1_mv500.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/expected_significance_vector_mx1_mv500_ablation_drop_extrajet.png}
  \caption{@@TOK:CAPF:6@@ Expected-significance scans versus the NLL discriminant threshold
...
@@TOK:ENVEND:f6@@\end{figure}

\section{@@TOK:HEADID:paper-head-19@@@@TOK:SECNUM:7. @@Robustness: extra-jet feature ablation}
...
(Section~@@TOK:REF:sec:validation@@) confirms that the ablation model is not
undertrained---every retained feature closes as well as in the baseline.

\begin{table}[t]
  \centering
  \caption{@@TOK:CAPT:4@@ Effect of dropping the extra-jet kinematic features. The ablation
  working point is selected with the same $B\geq20$ floor as the baseline.}
  \label{tab:ablation-results}
  \begin{tabular}{>{\raggedright\arraybackslash}p{2.6cm}rrrrrrr}
    \toprule
    ...
    \bottomrule
  \end{tabular}
@@TOK:ENVEND:t4@@\end{table}

Table~@@TOK:REF:tab:ablation-results@@ shows that removing detailed extra-jet
...
(Figure~@@TOK:REF:fig:signal-vs-background-nll@@): in the ablation model the
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/signal_vs_background_nll.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/signal_vs_background_nll_ablation_drop_extrajet.png}
  \caption{@@TOK:CAPF:7@@ Signal-versus-background NLL distributions for the baseline
...
@@TOK:ENVEND:f7@@\end{figure}

\section{@@TOK:HEADID:paper-sec-limitations@@@@TOK:SECNUM:8. @@Limitations}
...
detector systematic uncertainties @@TOK:CITE:delphes3@@. The proxy efficiency is
...
@@TOK:CITE:delphes3@@, which is designed for phenomenological studies rather than
...
or a full trigger emulation @@TOK:CITE:delphes3@@. The Delphes validation against
CMS and ATLAS shows good agreement for jet and \MET{} resolutions at
$p_T > 30~\GeV$ @@TOK:CITE:delphes3@@, which is the regime relevant to the
...
for which more accurate tools are required @@TOK:CITE:delphes3@@.
...
\section{@@TOK:HEADID:paper-head-20@@@@TOK:SECNUM:9. @@Conclusion}
...
@@TOK:CITE:rasineni_nsf_monoz@@ in three ways. Methodologically, it replaces the
...
channel and dataset @@TOK:CITE:rasineni_nsf_monoz@@.

\section*{@@TOK:HEADID:paper-declarations@@Declarations}
...
\noindent\textbf{Clinical trial registration:} Not applicable. This study does not involve clinical trials.
...
\section{@@TOK:HEADID:paper-app-hlt@@@@TOK:SECNUM:10. @@HLT paths and trigger details}
...
$p_T>80$~\GeV), as defined in Section~@@TOK:REF:sec:trigger-proxy@@.
...
\section{@@TOK:HEADID:paper-app-features@@@@TOK:SECNUM:A. @@Feature definitions and extraction formulas}
...
\begin{table}\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.28\textwidth}>{\raggedright\arraybackslash}p{0.67\textwidth}@{}}
\caption{@@TOK:CAPT:5@@ Feature schema (41 branches) and extraction formulas. Nine columns excluded from training are annotated ``(not trained)''.}\label{tab:feature-formulas}
\toprule
Feature & Formula \\
\midrule
...
\texttt{trigger\_pass} & Binary HLT decision: $\mathbb{1}[\text{event fires}\geq1\text{ path in App.~@@TOK:REF:app:hlt@@}]$ (via \texttt{pathNames\_} or \texttt{filterLabels\_} fallback); not trained (trigger flag) \\
...
\end{tabular}@@TOK:ENVEND:t5@@\end{table}
\normalsize



\section{@@TOK:HEADID:paper-app-filecounts@@@@TOK:SECNUM:B. @@Per-file event counts and audit}
...
(Section~@@TOK:REF:sec:reproducibility@@).

\subsection{@@TOK:HEADID:paper-head-21@@@@TOK:SECNUM:B.1 @@Data-side cutflow}

Table~@@TOK:REF:tab:data-cutflow@@ summarises the data-side event selection
stages. The trigger stage applies the five unprescaled HLT paths of
Appendix~@@TOK:REF:app:hlt@@; the offline stage applies the hadronic mono-$Z$
selection described in Section~@@TOK:REF:sec:data@@ (jet $p_T$ and $\eta$
...
\begin{table}[H]
  \centering
  \caption{@@TOK:CAPT:6@@ Data-side cutflow for the CMS Run~2015D HTMHT open dataset.}
  \label{tab:data-cutflow}
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.55\columnwidth}rr@{}}
    \toprule
...
Section~@@TOK:REF:sec:limitations@@); the final offline post-selection yields
for the three benchmarks are 116, 487, and 848 events, respectively.

\section{@@TOK:HEADID:paper-app-closures@@@@TOK:SECNUM:C. @@Full closure figures}
...
and $\Delta R_{Zjj}$ appear in Figure~@@TOK:REF:fig:closure-representative@@;
additional kinematic closure panels appear in
Figure~@@TOK:REF:fig:closure-kinematics@@; the sentinel-imputation panels appear
in Figure~@@TOK:REF:fig:imputation-comparison@@; the validation-NLL curves appear
in Figure~@@TOK:REF:fig:validation-nll@@; and the sensitivity and
signal-versus-background NLL figures appear in
Figures~@@TOK:REF:fig:sensitivity-scans@@ and~@@TOK:REF:fig:signal-vs-background-nll@@.
The remaining per-feature closure panels are collected below.

\subsection*{@@TOK:HEADID:paper-head-22@@Remaining baseline closure figures}

\begin{figure}[p]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_met_phi.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_dphi_met_jet1.png}\\[6pt]
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_dphi_met_jet2.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_min_dphi_met_jets.png}
  \caption{@@TOK:CAPF:8@@ Baseline closure plots for the missing-momentum observables and the undefined extra-jet angular features.}
@@TOK:ENVEND:f8@@\end{figure}

\begin{figure}[p]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_met_over_sqrtHT.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_met_over_ptZ.png}\\[6pt]
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_abs_met_minus_ptZ.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_abs_mjj_minus_mZ.png}
  \caption{@@TOK:CAPF:9@@ Baseline closure plots for the MET-to-scale ratios and angular proximity summary.}
@@TOK:ENVEND:f9@@\end{figure}

\begin{figure}[p]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_m_jj_focus.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_pt_Z.png}\\[6pt]
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_phi_Z.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_eta_Z_pseudo.png}
  \caption{@@TOK:CAPF:10@@ Baseline closure plots for the hadronic-$Z$ mass window observables and reconstructed $Z$ kinematics.}
@@TOK:ENVEND:f10@@\end{figure}

\begin{figure}[p]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_dphi_met_Z.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_deltaphi_zjj.png}\\[6pt]
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_hadronic_recoil_pt.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_u_parallel.png}
  \caption{@@TOK:CAPF:11@@ Baseline closure plots for the reconstructed $Z$ direction, dijet geometry, and recoil decomposition.}
@@TOK:ENVEND:f11@@\end{figure}

\begin{figure}[p]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_u_perp.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_u_parallel_over_ptZ.png}\\[6pt]
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_u_perp_over_ptZ.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_zjet1_pt.png}
  \caption{@@TOK:CAPF:12@@ Baseline closure plots for the recoil components and leading $Z$-daughter jet kinematics.}
@@TOK:ENVEND:f12@@\end{figure}

\begin{figure}[p]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_zjet2_pt.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_jet1_pt.png}\\[6pt]
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_jet2_pt.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_jet2_eta.png}
  \caption{@@TOK:CAPF:13@@ Baseline closure plots for the $Z$-daughter and extra-jet $p_T$ observables.}
@@TOK:ENVEND:f13@@\end{figure}

\begin{figure}[p]
  \centering
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_n_jets.png}\hfill
  \includegraphics[width=0.48\textwidth]{/research/z-hadronic/assets/images/closure_n_extra_jets.png}
  \caption{@@TOK:CAPF:14@@ Baseline closure plots for the jet-multiplicity observables.}
@@TOK:ENVEND:f14@@\end{figure}


\section{@@TOK:HEADID:paper-sec-reproducibility@@@@TOK:SECNUM:D. @@Reproducibility and code availability}
